...
$query="SELECT * FROM response WHERE `response_ID`='{$_REQUEST['responseID']}'";
$result=mysql_query($query) or die (mysql_error());
$fetchedData=mysql_fetch_array($result);
$_SESSION['newResponseData']=$fetchedData;
?>
<tr>
    <td height="104" class="rightTitleCell">
            <p>New Response from <?php echo $_SESSION['newResponseData']['field_user']?> in Questionnaire <?php echo $_SESSION['newResponseData']['application_ID'];?></p>
    </td>
</tr>
<tr>
    <td class="subCatCells">
        <?php
            $query="SELECT * FROM query WHERE `response_ID`='{$_SESSION['newResponseData']['response_ID']}'";
            $result=mysql_query($query) or die (mysql_error());
            $count=0;
            while($fetchedFromDB=mysql_fetch_array($result))
            {
                $fetchedQuestions[$count]=$fetchedFromDB;
                $count++;
            }
            $count=0;
            while($count<count($fetchedQuestions))
            {
                echo "<div class=\"questions\">";
                $qNo=$fetchedQuestions[$count]['question_no']+1;
                echo "Question $qNo : {$fetchedQuestions[$count]['text']}";
                echo "</div>";
                $query="SELECT * FROM answer WHERE `query_ID`='{$fetchedQuestions[$count]['query_ID']}'";
                $result=mysql_query($query) or die(mysql_error());
                $c2=0;
                while($fetchedFromDB=mysql_fetch_array($result))
                {
                    $fetchedAnswers[$c2]=$fetchedFromDB;
                    $c2++;
                }
                $c2=0;
                while($c2<count($fetchedAnswers))
                {
                    echo "<div class=\"answers\">";
                    echo "Answer {$fetchedAnswers[$c2]['rank']} : {$fetchedAnswers[$c2]['text']}";
                    echo "</div>";
                    $c2++;
                }
                $c2=0;
