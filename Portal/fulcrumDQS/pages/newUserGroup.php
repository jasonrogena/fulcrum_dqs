...
if(isset ($_REQUEST['submit']))
{
    ...
    $query="SELECT count(`group_name`) FROM `group` WHERE `group_name`='$groupName'";
    ...
    if($flag==0)
    {
        echo "<meta http-equiv=\"refresh\" content=\"0;URL=../phpScripts/addUserGroup.php\" />";
    }
}
else
{
...
<tr>
    <td class="subCatCells">
        <form action="newUserGroup.php" method="post">
            <table>
                <tr>
                        ...
                        <select name="selectApp" style="width:280px">
                            ...
                            <option></option>
                                <?php
                                    ...
                                    {
                                        echo "<option value=\"{$queryContent['application_ID']}\">".$queryContent['application_ID']." - ".$queryContent['name']."</option>";//application ID before name to avoid errors
                                    }
                                ?>
                        </select>
                    </td>
                ...
                <tr>
                    <td colspan="3" id="fieldUserCellID">
                            <?php
                                $query="SELECT username,`first_name`,surname
                                    FROM `field_user`
                                    WHERE `developer_username`='{$_COOKIE['developerUsername']}' AND `group_ID` IS NULL";
                                $result=mysql_query($query) or die(mysql_error());
                                $fetchedFUsers=mysql_fetch_array($result);
                                if($fetchedFUsers)
                                {
                                    echo '<fieldset><legend>Select Field Users to add to this Group</legend>';
